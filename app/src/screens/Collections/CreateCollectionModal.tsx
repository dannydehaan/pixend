...
export const CreateCollectionModal = ({ open, onClose, onSuccess }: Props) => {
  ...
  const canSubmit = Boolean(workspaceId) && name.trim().length > 0;

  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!workspaceId) {
      setGlobalError("Select a workspace first");
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      ...
      >
        <header>
          <p className="text-xs uppercase tracking-[0.4em] text-on-surface-variant">New Collection</p>
          <h2 className="text-2xl font-semibold">Create a collection for your workspace</h2>
        </header>
        {globalError && <p className="text-sm text-error/80">{globalError}</p>}
        <div className="space-y-3">
          <label className="text-xs uppercase tracking-[0.4em] text-on-surface-variant" htmlFor="collection-workspace">
            Workspace
          </label>
          <select
        ...
        <div className="space-y-3">
          <label className="text-xs uppercase tracking-[0.4em] text-on-surface-variant" htmlFor="collection-name">
            Name
          </label>
          <input
            id="collection-name"
        ...
        <div className="space-y-3">
          <label className="text-xs uppercase tracking-[0.4em] text-on-surface-variant" htmlFor="collection-description">
            Description
          </label>
          <textarea
